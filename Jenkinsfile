...
pipeline {
  ...
  agent {
    docker{
      ...
      args '--network dev-net -v shared-war:/shared-war -v gradle-cache:/home/gradle/.gradle -v /var/run/docker.sock:/var/run/docker.sock'
     ...
     steps {
      ...
      echo "SONAR_TOKEN (first 6 chars): ${SONAR_TOKEN[0..5]}"
       ...
       sh '''
         ...
         ./gradlew checkQualityGate -Dsonar.login=${SONAR_TOKEN}
       '''
     }
   }
